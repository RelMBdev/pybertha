...
def mtxto_npcmplxarray (realp, imagp):
    ...
    for i in range(len(list_REAL)):
        ...
        row = tf.zeros(len(list_REAL[i]), dtype=np.complex128)
...
def main_loop (D_ti, fock_mid_backwd, j, dt, H, I, dip_mat, C, C_inv, S, nbf, \
    ...
    Ah=tf.math.conj(fock_mid_tmp.T)
    fo.write('Fock_mid hermitian: %s\n' % np.allclose(fock_mid_tmp,Ah))
    #transform fock_mid_init in MO basis
    fockp_mid_tmp=tf.matmul(tf.math.conj(C.T),tf.matmul(fock_mid_tmp,C))
    u=util.exp_opmat(tf.cast(tf.identity(np.copy(fockp_mid_tmp)),dtype=tf.complex128),dt)
    #u=scipy.linalg.expm(-1.0j*fockp_mid_tmp*dt)
    #check u is unitary
    test_u=tf.matmul(u,tf.math.conj(u.T))
    if (not np.allclose(tf.eye(u.shape[0], dtype=tf.complex128),test_u)):
        print('U is not unitary\n')
    
    #check the trace of density to evolve
    fo.write('tr of density to evolve: %.8f\n' % tf.math.real(tf.linalg.trace(Dp_ti)))
    
    #evolve the density in orthonormal basis
    temp=tf.matmul(Dp_ti,tf.math.conj(u.T))
    Dp_ti_dt=tf.matmul(u,temp)
    
    #backtransform Dp_ti_dt
    D_ti_dt=tf.matmul(C,tf.matmul(Dp_ti_dt, tf.math.conj(C.T)))
    fo.write('%.8f\n' % tf.math.real(tf.linalg.trace(Dp_ti_dt)))
    #dipole expectation for D_ti
    dip_list.append(tf.linalg.trace(tf.matmul(dip_mat,D_ti)))
    ...
    if (func=='hf'):
        ene_list.append(tf.linalg.trace(tf.matmul(D_ti,(H+F_ti))))
    else:
        ene_list.append(2.00*tf.linalg.trace(tf.matmul(D_ti,H))+J_i+Exc_i-tf.linalg.trace(tf.matmul(D_ti,(func_ti*dip_mat))))
    ...
    D_ti=tf.cast(tf.identity(np.copy(D_ti_dt)),dtype=tf.complex128)
    Dp_ti=tf.cast(tf.identity(np.copy(Dp_ti_dt)),dtype=tf.complex128)
    #update fock_mid_backwd for the next step
    fock_mid_backwd=tf.cast(tf.identity(np.copy(fock_mid_tmp)),dtype=tf.complex128)
...
def normal_run_init (args):
    ...
    S = tf.convert_to_tensor(np.array(mints.ao_overlap()),dtype=tf.complex128)
    #get T,V,dipole_z
    T = tf.convert_to_tensor(np.array(mints.ao_kinetic()),dtype=tf.complex128)
    V = tf.convert_to_tensor(np.array(mints.ao_potential()),dtype=tf.complex128)
    dipole=mints.ao_dipole()
    #dipole is  a list of psi4.core.Matrix objects
    # dipole[0] -> x, dipole[1] -> y, dipole[2] -> z
    dip_mat=tf.cast(tf.identity(np.copy(dipole[direction])),dtype=tf.complex128)
    H = T+V
    ...
    #C coefficients
    C=tf.convert_to_tensor(np.array(wfn.Ca()),dtype=tf.complex128)  
    
    dipmo_mat = tf.matmul(tf.math.conj(C.T),tf.matmul(dip_mat,C))
    #get a scf alpha density
    Da = tf.convert_to_tensor(np.array(wfn.Da()),dtype=tf.complex128)
    ...
    I = tf.convert_to_tensor(np.array(mints.ao_eri()),dtype=tf.complex128)
    
    #D_0 is the density in the reference molecular basis
    D_0_=np.zeros((nbf,nbf))
    for num in range(int(ndocc)):
        D_0_[num,num]=1.0
    D_0=tf.convert_to_tensor(D_0_,dtype=tf.complex128)
    ...
    if (analytic):
        ...
        dip_mo=tf.matmul(tf.math.conj(C.T),tf.matmul(dip_mat,C))
        ...
        Dp_init= tf.matmul(u0,tf.matmul(Dp_0, tf.math.conj(u0.T)))
        func_t0 = k
        #backtrasform Dp_init
        D_init = tf.matmul(C,tf.matmul(Dp_init, tf.math.conj(C.T)))
        Da = D_init
        Dp_0 = Dp_init 
       
        #J0p,Exc0p,F_t0=util.get_Fock(D_ti,H,I,func,basisset)
        #if (func == 'hf'):                                  
        #    testene = tf.linalg.trace(tf.matmul(D_init,(H+F_t0)))  
        #else:                                               
        #    testene = 2.00*tf.linalg.trace(tf.matmul(D_init,H))+J0p+Exc0p
        #print('trace D(0+): %.8f' % tf.linalg.trace(Dp_init).real)       
    ...
    Ah=tf.math.conj(fock_mid_init.T)
    fo.write('Fock_mid hermitian: %s\n' % np.allclose(fock_mid_init,Ah))
    
    #propagate D_t0 -->D(t0+dt)
    #
    #fock_mid_init is transformed in the MO ref basis
    fockp_mid_init = tf.matmul(tf.math.conj(C.T),tf.matmul(fock_mid_init,C))
    
    #u=scipy.linalg.expm(-1.j*fockp_mid_init*dt)
    u = util.exp_opmat(fockp_mid_init,dt)
    
    temp = tf.matmul(Dp_0,tf.math.conj(u.T))
    
    Dp_t1 = tf.matmul(u,temp)
    
    #check u if unitary
    test_u = tf.matmul(u,tf.math.conj(u.T))
    fo.write('U is unitary :%s\n' % tf.reduce_all(tf.abs(test_u - tf.eye(u.shape[0], dtype=tf.complex64)) < 1e-7))
    
    fock_mid_backwd = tf.cast(tf.identity(np.copy(fock_mid_init)),dtype=tf.complex128)
    
    #backtrasform Dp_t1
    
    D_t1=tf.matmul(C,tf.matmul(Dp_t1, tf.math.conj(C.T)))
    
    if (func == 'hf'):
        ene_list.append(tf.linalg.trace(tf.matmul(Da,(H+F_t0))))
    else:    
        ene_list.append(2.00*tf.linalg.trace(tf.matmul(Da,H))+J0+Exc0-tf.linalg.trace(tf.matmul(Da,(func_t0*dip_mat))))
    
    dip_list.append(tf.linalg.trace(tf.matmul(Da,dip_mat)))
    ...
    fock_mid_backwd=tf.cast(tf.identity(np.copy(fock_mid_init)),dtype=tf.complex128) #prepare the fock at the previous midpint
    D_ti=D_t1
    Dp_ti=Dp_t1
    ...
    if debug :  
        #trace of D_t1
        fo.write('%.8f\n' % tf.math.real(tf.linalg.trace(Dp_ti)))
        fo.write('Trace of DS %.8f\n' % tf.math.real(tf.linalg.trace(tf.matmul(S,D_ti))))
        fo.write('Trace of SD.real %.14f\n' % tf.linalg.trace(tf.matmul(tf.math.real(S),tf.math.real(D_ti))))
        fo.write('Trace of SD.imag %.14f\n' % tf.linalg.trace(tf.matmul(tf.math.real(S),tf.math.imag(D_ti))))
...
def restart_init (args):
    ...
    dipmo_mat = tf.convert_to_tensor(np.array(json_data["dipmo_mat"]),dtype=tf.complex128)
    H = tf.convert_to_tensor(np.array(json_data["H"]),dtype=tf.complex128)
    I = tf.convert_to_tensor(np.array(json_data["I"]),dtype=tf.complex128)
    dip_mat = tf.convert_to_tensor(np.array(json_data["dip_mat"]),dtype=tf.complex128)
    C = tf.convert_to_tensor(np.array(json_data["C"]),dtype=tf.complex128)
    C_inv = tf.convert_to_tensor(np.array(json_data["C_inv"]),dtype=tf.complex128)
    S = tf.convert_to_tensor(np.array(json_data["S"]),dtype=tf.complex128)
